...
from flatpunch import z_eq, force_by_parts, Ediss_by_parts
import numpy as np
...
Amp = summ[:,8]*1.0e-9  #reduced tapping amplitude in nanometers
...
nu = summ[0,4]
Ediss_compu = summ[:,11]*1.0e-18  #dissipated energy calculated in the c code (numerically)
...
E_diss_an = np.zeros(np.size(files))
E_loss= np.zeros(np.size(files))
E_store= np.zeros(np.size(files))
E_tr= np.zeros(np.size(files))
E_relax= np.zeros(np.size(files))
E_vdw= np.zeros(np.size(files))
...
for f in files:
    ...
    k = zlist.index(number)
    zeq[k] = z_eq(force, tip, Amp[k], 25.0)
    _, t_in, t_fin, _, _, _, _, _, _ = \
    force_by_parts(zeq[k], Amp[k], R, freq, G, tau, Ge, nu, 2.0e-19, 10000, 1.0)
    E_diss_an[k], E_loss[k], E_store[k], E_tr[k], E_relax[k], E_vdw[k] = Ediss_by_parts(t_in, t_fin, zeq[k], Amp[k], R, freq, G, tau, Ge, nu, 2.0e-19, 10000, 1.0)
    i=i+1
...
axes[0].plot(A_ratio, Ediss_compu*1.0e18,'-^',color='dimgray', markersize=8.0, lw=1.0, label = 'Simulation')
axes[0].plot(A_ratio, E_diss_an*1.0e18, 'k-*', markersize=10.0, lw=1.0, label = 'Analytical')
axes[0].set_ylabel('Total dissipation, aJ', color='k', fontsize='18')
...
axes[1].plot(A_ratio, E_loss*1.0e18, 'b-o',markersize=7.0, lw=1.0, label = 'Loss Modulus')
axes[1].plot(A_ratio, E_store*1.0e18, 'b-D',markersize=7.0, lw=1.0, label = 'Storage Modulus')
...
axes[1].set_ylim(-100,850)
axes[1].legend(loc=3, fancybox=True, framealpha=0, fontsize=11)


axes[2].plot(A_ratio, E_tr*1.0e18, 'g-x', markeredgewidth=2.0, markersize=8.0, lw=1.0, label = 'Transients')
axes[2].plot(A_ratio, E_relax*1.0e18, 'g-s', lw=1.0,markersize=7.0, label = 'Relaxation Modulus')
axes[2].set_ylabel('Transients and Relaxation, aJ', color='green', fontsize='18')
axes[2].tick_params(axis='y', colors = 'green')
axes[2].spines['right'].set_color('green')
axes[2].spines['right'].set_linewidth(3.0)
axes[2].set_ylim(-9000,8000)
axes[2].legend(loc=4, fancybox=True, framealpha=0, fontsize=11)

#ax.legend(loc=3, fancybox=True, framealpha=0, fontsize=11)
...
plt.savefig('Ediss.png', bbox_inches='tight')
